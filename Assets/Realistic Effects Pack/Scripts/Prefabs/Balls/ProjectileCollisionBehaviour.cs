...
public class ProjectileCollisionBehaviour : MonoBehaviour
{
  ...
  private void InitializeDefault()
  {
    ...
    if (!effectSettings.UseMoveVector) {
      forwardDirection = tRoot.position + (tTarget.position - tRoot.position).normalized * effectSettings.MoveDistance;
      GetTargetHit();
    }
    else
  ...
  private void Update()
  {
    ...
    var moveDistance = Vector3.MoveTowards(tRoot.position, new Vector3(endPoint.x, 1.0f, endPoint.z), effectSettings.MoveSpeed * Time.deltaTime * accelerationY);
